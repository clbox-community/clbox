...
import { ResponseAssessmentResult, UserAssessmentVerification } from 'assessment-model';
...
describe('assessment result', () => {
    ...
    it('should correctly mark numeric questions', () => {
        expect(assessmentResponseAssessResult(Seniority.senior, q({
          senior: [4],
          seniorPlus: [4]
        }), 1, {})).toBe(ResponseAssessmentResult.NotExpectedRequired);
        expect(assessmentResponseAssessResult(Seniority.senior, q({
          senior: [4],
          seniorPlus: [4]
        }), 2, {})).toBe(ResponseAssessmentResult.NotExpectedRequired);
        expect(assessmentResponseAssessResult(Seniority.senior, q({
          senior: [3, 4],
          seniorPlus: [4]
        }), 3, {})).toBe(ResponseAssessmentResult.NotExpectedNotRequired);
        expect(assessmentResponseAssessResult(Seniority.senior, q({
          senior: [3, 4],
          seniorPlus: [4]
        }), 4, {})).toBe(ResponseAssessmentResult.ExpectedResponse);
        expect(assessmentResponseAssessResult(Seniority.senior, q({
          senior: [3, 4],
          seniorPlus: [4]
        }), undefined, {})).toBe(ResponseAssessmentResult.NotAsked);
    })
});
